"""
Modified version of Agent-Aware Attentio module for social-aware attention
"""
...
def agent_aware_attention(query: Tensor,
    ...
    if not use_separate_proj_weight:
        if torch.equal(nan_to_num(query), nan_to_num(key)) and torch.equal(nan_to_num(key), nan_to_num(value)):
            # self-attention
        ...
        elif torch.equal(nan_to_num(key), nan_to_num(value)):
    ...
    if in_proj_weight_self is not None:
        ...
        # learnable connectivity mask   
        if pos_mask is not None:
            connectivity_mask = learnable_mask.unsqueeze(1) * pos_mask 
            connectivity_mask = torch.sum(connectivity_mask, dim=0)
            _pos_mask = (1 - attn_weight_self_mask) * connectivity_mask 
        else:
            _pos_mask = (1 - attn_weight_self_mask)
            
        # agent wise attention
        attn_output_weights = attn_output_weights_inter * _pos_mask + attn_output_weights_self * attn_weight_self_mask
...
class AgentAwareAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
        ...
        agent_num = 6
        input_length = 12
        # self.learnable_mask = Parameter(torch.empty(agent_num * input_length, agent_num * input_length))
        # self.learnable_mask = Parameter(torch.rand(agent_num * input_length, agent_num * input_length))
        self.learnable_mask = Parameter(torch.rand(3, agent_num * input_length, agent_num * input_length))
    ...
    def _reset_parameters(self):
        ...
        # xavier_uniform_(self.learnable_mask)
    ...
    def forward(self, query, key, value, key_padding_mask=None,
                need_weights=True, attn_mask=None, num_agent=1, pos_mask=None):
        r"""
    Args:
        query, key, value: map a query and a set of key-value pairs to an output.
            See "Attention Is All You Need" for more details.
...
class AgentFormerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        super().__init__()
        self.self_attn = AgentAwareAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, num_agent=1, pos_mask=None) -> Tensor:
        ...
        src2 = self.self_attn(src, src, src, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask, num_agent=num_agent, pos_mask=pos_mask)[0]
...
class AgentFormerDecoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        super().__init__()
        self.self_attn = AgentAwareAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = AgentAwareAttention(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = Linear(d_model, dim_feedforward)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model)
...
class AgentFormerEncoder(Module):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, num_agent=1, pos_mask=None) -> Tensor:
        ...
        for mod in self.layers:
            output = mod(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, num_agent=num_agent, pos_mask=pos_mask)
...
class AgentFormerDecoder(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        Args:
            tgt: the sequence to the decoder (required). 未来轨迹
            memory: the sequence from the last layer of the encoder (required). context vector
            tgt_mask: the mask for the tgt sequence (optional).
            memory_mask: the mask for the memory sequence (optional).
            tgt_key_padding_mask: the mask for the tgt keys per batch (optional).
            memory_key_padding_mask: the mask for the memory keys per batch (optional).
